...
///-------------------------------------
#include </Users/user/CLionProjects/VoroCAnalyser22/src/eigen3/Eigen/Sparse>
#include </Users/user/CLionProjects/VoroCAnalyser22/src/eigen3/Eigen/Eigenvalues>
///-------------------------------------
...
/// Adjacency sparse matrix for grains
    i = 0, j = 0, value = 0; tripletList.clear();
    ifstream inAG(AG);
    if (inAG.is_open()) { //If the file was successfully open, then
        while(!inAG.eof()) {
            inAG >> i >> j >> value;
            tripletList.push_back(Tr(i, j, value));
            }
    } else cout << "The file " << AG << " cannot be read" << endl; //If something goes wrong
//Sparse AG matrix
    SpMat AGS(CellNumbs.at(3) + 1,CellNumbs.at(3) + 1);
    AGS.setFromTriplets(tripletList.begin(), tripletList.end());
...
/// Incidence sparse matrix for Grains and Faces
    i = 0, j = 0, value = 0; tripletList.clear();
    ifstream inMGF(MGF);
    if (inMGF.is_open()) { //If the file was successfully open, then
        while(!inMGF.eof()) {
            inMGF >> i >> j >> value;
            tripletList.push_back(Tr(i, j, value));
            }
    } else cout << "The file " << MGF << " cannot be read" << endl; //If something goes wrong
//Sparse MGF matrix
    SpMat GFS(CellNumbs.at(3) + 1,CellNumbs.at(2) + 1);;
    GFS.setFromTriplets(tripletList.begin(), tripletList.end());
...
/// =================>  Random generation process   <==============================////
       ...
       /// Step for output (proportional to the number of iterations made)
        int output_step = 300;
        do { // Loop over all possible fractions of special cells [0,1]
...
//                SpMat FES(CellNumbs.at(2) + 1,CellNumbs.at(1) + 1);
                ...
                SpMat FES_Graph;
                FES_Graph*FES_Graph.transpose() = SAM_FacesGraph + SFDegree;

                if (configuration.at(0) == 1) { // Nodes types statistics, indices and configuration entropy
                    //#include "QPsLab.h"
                } else if (configuration.at(1) == 1) { // Edges types statistics, indices and configuration entropy
                    //#include "TJsLab.h"
                } else if (configuration.at(2) == 1) { // Faces types statistics and structural indices
                    //#include "FacesLab.h"
                } else if (configuration.at(3) == 1) { // Grains types statistics, indices and configuration entropy
                    //#include "GrainsLab.h"
                } else if (configuration.at(4) == 1) { // Nodes Laplacian with its spectrum for the nodes network
                    //#include"NodeLaplacians.h"
                } else if (configuration.at(5) == 1) { // Edges Laplacian with its spectrum for the edges network
                    //#include"EdgeLaplacians.h"
                } else if (configuration.at(6) == 1) { // Face Laplacian with its spectrum for the special faces network
                    //#include"FaceLaplacians.h"
                    SpMat Laplacian_SFaces;
                    Laplacian_SFaces = FES_Graph * FES_Graph.transpose();

                    /// Eigenvalues of the S-Faces laplacian
                    MatrixXd LapFEig(numerator, numerator);
                    EigenSolver<MatrixXd> es(Laplacian_SFaces);
                    cout<<"es(LapFEig) just calculated"<<"    size = "<< es.eigenvalues().size() <<endl;
                    MatrixXcd EigF = es.eigenvalues();

//                        cout << Laplacian_specialFaces.nonZeros() << endl;
                    // EigenSolver<SpMat> lu(Laplacian_specialFaces);
                    //Laplacian_Eigenvalues(Laplacian_specialFaces);
...
//                        cout << Laplacian_specialFaces << endl;
                } else if (configuration.at(7) == 1) { // Betti numbers (FB0, FB1 and FB2) of the special faces network
                    //#include"BettiNumbers.h"
                } else if (configuration.at(8) == 1) { // Grain Laplacian with its spectrum for the grain network
                    //#include"GrainLaplacians.h"
                } else if (configuration.at(9) == 1) { // Tutte polynomial for the special faces network
                    //#include"FaceTutte.h"
                } else if (configuration.at(10) == 1) { // Statistical physics module
